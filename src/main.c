...
static void pixan_set_pixan_routes (HttpCerver *http_cerver) {

	/* register top level route */
	// GET /pixan
	HttpRoute *main_route = http_route_create (REQUEST_METHOD_GET, "pixan", pixan_handler);
	http_cerver_route_register (http_cerver, main_route);

	/* register pixan children routes */
	// GET pixan/version
	HttpRoute *version_route = http_route_create (REQUEST_METHOD_GET, "version", pixan_version_handler);
...
static void start (void) {
	...
	if (test_api) {
		...
		pixan_set_pixan_routes (http_cerver);

		// add a catch all route
		http_cerver_set_catch_all_route (http_cerver, pixan_catch_all_handler);

		// admin configuration
		http_cerver_enable_admin_routes (http_cerver, true);
		http_cerver_register_admin_file_system (http_cerver, "/var/uploads");
		http_cerver_register_admin_file_system (http_cerver, "/home/pixan/uploads");
...
int main (int argc, char const **argv) {
	...
	if (!pixan_init ()) {
		...
		cerver_log_error ("Failed to init pixan!");
	}
